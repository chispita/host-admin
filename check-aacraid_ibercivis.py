...
# Chris Recktenwald
#   patch: verbose mode
#   fix:   parse "ZMM Optimal" correctly
#
# check-aacraid.py [-v]
#
...
for line in os.popen4("sudo /usr/local/admin/arcconf GETCONFIG 1 LD")[1].readlines():
	# Match the regexs
	...
	if lstatus:
		if lstatus.group(1) != "Optimal":
			if verbose > 0 :
				print "lstatus not Optimal"
			check_status = 2
		result += "Logical Device " + lnum + " " + lstatus.group(1) + ","

for line in os.popen4("sudo /usr/local/admin/arcconf GETCONFIG 1 AD")[1].readlines():
	# Match the regexs
	cstatus = c_status_re.match(line)
	if cstatus:
		if cstatus.group(1) != "Optimal":
			if verbose > 0 :
				print "cstatus not Optimal"
			check_status = 2
	...
	if bstatus:
		...
		if bstatus.group(1) == "Charging":
			if check_status < 2:
				if verbose > 0 :
					print "bstatus Charging"
				check_status = 1
		elif bstatus.group(1) != "Optimal" and bstatus.group(1) != "ZMM Optimal" :
			if verbose > 0 :
				print "bstatus not Optimal"
			check_status = 2
